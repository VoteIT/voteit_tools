...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "-t", help="Tagg för val av dagordningspunkter", type=str, required=True
        )
        parser.add_argument(
    ...
    def handle(self, *args, **options):
        ...
        ai_qs = meeting.agenda_items.filter(tags__contains=[tag])
        if ai_count := ai_qs.count():
            self.stdout.write(f"Processing {ai_count} agenda items")
        else:
            self.stdout.write(self.style.WARNING("No agenda items match tag"))
            exit("Nothing to do")
